...
**v2.50-sangala** (will ship as pre-release; promote manually on GitHub Releases after device test passes). Queued for v2.50: **the dictionary-in-update-package fix** (Lesson #31) — workflow change so `-update.tar.gz` no longer ships the StarDict source files. Device test should specifically verify (a) a Wiktionary lookup after a fresh install+update gives a coherent definition with no raw `[en]`/`[de]` markers, AND (b) a standalone update against an already-installed device still lands the new Plato build but does not touch the dictionary directory (compare mtimes on `dictionary.dict.dz` / `dictionary.index` before vs. after the update).
