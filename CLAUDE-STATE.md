...
3. **12-hour clock with AM/PM, simplified clock-menu.** Top-bar clock now shows `3:04 PM` rather than `15:04` (default `time_format` changed to `"%-I:%M %p"` in `crates/core/src/settings/mod.rs` and overridden in `sangala/Settings.toml`). Clock dropdown menu reworked to three submenus that all fit on screen without `More`-overflow: `AM / PM` (2 entries), `Set Hour` (12 entries, 1–12), `Set Minute` (12 entries, 00..55 step 5). Removed the date-format message header from the menu — date is not displayed or settable. New `SetTimeAmPm(bool)` event variant; `SetTimeHour` handler now maps a 1..=12 value to the 24h hour preserving the current AM/PM. This fixes three user-reported problems with the previous menu: 24h-only display, no AM/PM toggle, and the appearance that the hour list was "not sequential" (caused by the 24-entry hour list overflowing into nested `More` submenus during navigation — gone now because every submenu fits).
