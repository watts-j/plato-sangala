...
Last updated: 2026-05-12 (GUI installer verified + v2.49 prep)
...
## Session-End Handoff (2026-05-12, GUI verified + v2.49 prep)

**GUI installer verified on factory-reset Clara BW.** Fresh-install flow worked end-to-end (Phase 1 copy → eject → reconnect → Phase 2 copy → eject → "Update complete" with OK button) after fixing a timer-handler accumulation bug in `install-sangala-gui.ps1`.

**The bug (fixed in `4cb9cb5`).** Each phase transition did `$script:timer.Stop()` / `$script:timer.Remove_Tick({})` / `$script:timer.Add_Tick({ new handler })` / `$script:timer.Start()`. `Remove_Tick({})` passes a fresh empty scriptblock that doesn't equal the previously-attached delegate, so .NET never removed any handler. By Phase 2 the leftover Phase 1 `On-CopyTick` was still attached; when the Phase 2 copy completed, the Phase 1 handler fired first, invoked its captured `{ After-InstallStep1 }` callback, called `Start-Disconnect { Show-WaitingForReconnect }`, ejected the device again, waited for reconnect, and landed at `Show-DetectedUpdate` — the user saw "Existing Plato installation found. Click Update..." instead of "Update complete". The Phase 2 handler's success callback (`Start-Disconnect { Show-Done }`) never fired because Phase 1's handler had already cleared `$script:S.Bg`. **Fix:** one Tick handler attached for the lifetime of the form; it dispatches by `$script:S.Mode` to `On-CopyTick` / `On-DisconnectTick` / `On-ReconnectTick`. Phase transitions just set `Mode` + start the timer. Each tick function clears `Mode = 'idle'` and stops the timer before transitioning. Also stashes success callbacks in `$script:S.CopyOnSuccess` / `$script:S.DisconnectOnSuccess` so PowerShell scope/closure quirks with captured `$onSuccess` parameters can't bite.

**Also cleaned up:** the GUI's name-prompt status text used to claim the home screen would show `"Welcome, {name}!"`, which has been false since v2.45. Now says `The name will appear on the device's home screen, e.g. "John Smith".`. The same staleness existed in `install-sangala.ps1`'s comment header — fixed too (not user-visible but kept aligned).

**v2.49 is the next ship.** Adds the GUI as `install-sangala-gui.exe` inside `install-sangala.zip`. CI workflow now has a `gui-exe` job (`runs-on: windows-latest`, `needs: build`) that uses PS2EXE to compile `install-sangala-gui.ps1` into the .exe, repackages the zip with both the .ps1 and the .exe, and `gh release upload --clobber`s the build job's .ps1-only zip. The .exe path is the recommended one for non-technical users; the .ps1 stays for advanced users who want to inspect the script.

**Branch `claude/customize-plato-ui-1Edbm` tip:** see `git rev-parse origin/claude/customize-plato-ui-1Edbm`. **Latest tag:** `v2.48-sangala`. **Next tag:** `v2.49-sangala` (ready to push; not yet pushed). Pushing the tag triggers CI which builds tarballs + .exe + zip and creates a pre-release on GitHub. Verify CI succeeds, do an end-to-end device test against the released artifacts, then manually promote v2.49 from pre-release to "Latest" on GitHub.
...
1. ~~**GUI installer verification.**~~ **DONE.** Verified on factory-reset Clara BW (2026-05-12); ships as `.exe` in v2.49.
...
4. **GUI: auto-continue Phase 2 after reconnect.** Currently the GUI lands at `Show-DetectedUpdate` post-reconnect and waits for the user to click "Update" again before starting Phase 2. The CLI does it automatically. Considered fixing in this session, deferred. One-line change in `On-ReconnectTick`: replace `Show-DetectedUpdate` with `Start-UpdateOnly` (or a fresh-install-specific equivalent that auto-runs `Start-UpdateCopy`). Worth deciding before v2.50.
...
- **WinForms timer handlers in PowerShell** — see Lesson #24 below. Single-dispatcher pattern (one Add_Tick, switch by state) is far safer than per-phase Add/Remove churn. If anyone reintroduces per-phase `Add_Tick`/`Remove_Tick` calls, the Phase-2 reversion bug will come back.
