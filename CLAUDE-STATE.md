...
Last updated: 2026-05-25 (cable confirmed separable from reset bug; factory reset reproduces on a NEW Clara BW with v2.47 — eMMC/device-specific hypotheses are ruled out)
...
- **No version is currently confirmed stable.** As of 2026-05-25, factory reset on the ~2nd power cycle has now been reproduced on a **new, never-previously-reset Clara BW** running **v2.47** with a known-good USB cable. v2.47's actual contents (verified by reading the tag): boot-grace skip from v2.44 IS present; **Enable WiFi** is STILL in the burger menu; clock is 24-hour with date header and 5-minute granularity; GUI installer NOT in the zip (CLI .ps1 only). So the new-device v2.47 reset rules out: cable; eMMC wear / hardware fault on test device N365594030627; the GUI installer; the 12-hour clock UI changes. It does NOT rule out: WiFi-induced Nickel tamper-detect (still reachable from the menu in v2.47); the v2.44 boot-grace skip (present in v2.47, and previously tested as a revert in v2.53 without success — see Lesson #32). The v2.49 production batch of ~30 devices is presumed at-risk under multi-power-cycle (untested). See Session-End Handoff for the investigation history.
...
### Update 2026-05-25: factory reset reproduces on a new device with v2.47

After the cable swap (which DID resolve install-time frozen dots — Lesson #30 confirmed), a clean v2.47 install on a **brand-new Clara BW that had never been factory-reset before** factory-reset on the 2nd power cycle. v2.47 verified contents (from `git show v2.47-sangala`): v2.44 boot-grace skip present in `plato-autostart.sh`; **Enable WiFi** entry still in `toggle_main_menu`; 24-hour clock with date header and 5-minute granularity in `toggle_clock_menu`; install zip is CLI-only.
...
- **Cable ruled out for the reset** — new cable, still resets.
- **GUI installer ruled out** — v2.47 zip is CLI-only.
- **Post-v2.47 UI changes (12-hour clock, WiFi menu removal) ruled out** — v2.47 has none of them, still resets.

### Ruled out (don't re-investigate without new evidence)

- GUI installer specifically — v2.47 (CLI-only) also resets a new device.
- v2.44 boot-grace skip alone — reverting it in v2.53 did not stop the reset on the test device. (It IS present in v2.47, so the boot-grace path can't be cleared without a fresh full test.)
...
- 12-hour clock / removed date / removed WiFi menu — none of those changes exist in v2.47 yet it resets.

### Open hypotheses (ranked, best to worst)

1. **WiFi-induced Nickel tamper-detect**: Enable WiFi is reachable from the burger menu in v2.47 and the user may have toggled it on between the 1st and 2nd boot. Lesson #32 hypothesis (b) was disproven only as "devices reset without ever touching WiFi" — needs to be re-checked against the new-device test (was WiFi enabled in that run?). If WiFi was enabled, Nickel can run an OTA check during its pre-plato-autostart window and decide the system is tampered.
2. **Something in the install package or boot path interacts with Kobo firmware 4.42.23291's boot-integrity / tamper-detect.** Diff `plato-autostart.sh`, `plato.sh`, and KoboRoot.tgz between an actually-stable era (v2.30/v2.32) and v2.47 to see what shifted.
3. **The v2.44 boot-grace skip** — already tested in isolation via v2.53 revert and didn't help, but only on the heavily-reset N365594030627. Could be combined with something else on a new device.
4. **Different USB port on the host** — still untested, lower plausibility.
5. **Something we haven't thought of.** MobileRead, Kobo firmware notes.
...
The factory reset is reproducible across hardware (new Clara BW with v2.47). The bug exists in v2.47 or its install layout.

1. **Confirm whether Enable WiFi was toggled during the v2.47 new-device test** (between 1st and 2nd boot). v2.47 still has the burger-menu entry — if it was used, Nickel may have tamper-detected during its pre-plato-autostart window. If WiFi was NOT touched, the WiFi tamper hypothesis is finally dead.
2. **Diff `plato-autostart.sh`, `plato.sh`, and the KoboRoot.tgz contents between v2.30/v2.32 (eras user remembers as actually stable across reboots) and v2.47.** Look at kill order for `nickel`/`on-animator`, any new files inside KoboRoot.tgz, and whether the install package layout changed.
3. **Pull a device-state snapshot AFTER a successful 1st boot, BEFORE the 2nd power cycle.** If `.kobo/` has gained anything new, or `Settings.toml`/`Kobo eReader.conf` differs, that's a clue. Snapshot tool needs to live in `tools/` (Lesson #34).
...
32. **Kobo Clara BW devices factory-reset on boot after installing this build; root cause not yet found.** Symptom: device factory-resets on the ~2nd power cycle, enters recovery mode, lands at Kobo's OOBE wizard. **Hypotheses tested and disproven** as of 2026-05-25: (a) ~~GUI installer is the cause~~ (v2.49 CLI install resets the test device; v2.47 CLI-only also resets a new device); (b) ~~Anything we added since v2.24 is the cause~~ (v2.24 manual drag-drop also fails); (c) ~~Flaky USB cable causes the reset~~ (cable swap resolved install-time frozen dots — Lesson #30 — but device still reset on 2nd power cycle with the new cable); (d) ~~Test-device-specific hardware fault or eMMC wear on N365594030627~~ (a brand-new Clara BW that had never been factory-reset before, running v2.47 with a good cable, also factory-reset on the 2nd power cycle — see Lesson #35); (e) ~~Post-v2.47 UI changes are the cause~~ (v2.47 has neither the 12-hour clock nor the WiFi-menu removal, yet resets). **Still in v2.47 and not yet ruled out:** the v2.44 boot-grace skip (revert was tested only on the worn test device, never on a fresh one); Enable WiFi entry being reachable from the burger menu (could trip Nickel tamper-detect if toggled during the brief Nickel window); the install package layout; the boot-time kill/exec sequence. The bug is reproducible across hardware. Cannot get device-side logs after a reset (user partition wiped). Next-step direction: confirm whether WiFi was toggled in the new-device test; diff `plato-autostart.sh`/`plato.sh`/KoboRoot.tgz between v2.30/v2.32 and v2.47; pull a device snapshot between 1st boot success and 2nd-boot reset; search MobileRead for Clara BW + firmware 4.42.23291 recovery behavior. Do not tag any v2.5x as stable until at least one device survives 10+ power cycles.
...
35. **USB cable swap separates two distinct failure modes; the reset reproduces on fresh hardware with v2.47.** (2026-05-25) A cable swap resolved install-time frozen-dots (Lesson #30 pattern confirmed). Then a clean v2.47 install on a **brand-new Clara BW** (never previously factory-reset) with the good cable still factory-reset on the 2nd power cycle. v2.47 verified contents: v2.44 boot-grace skip present; **Enable WiFi** entry IS in the burger menu; 24-hour clock with date header and 5-minute granularity; CLI-only install zip (no GUI .exe yet). Implications: (a) frozen-dots-at-install and factory-reset-on-reboot are two separate bugs; (b) eMMC wear / device-specific hardware fault on N365594030627 is ruled out; (c) GUI installer is ruled out; (d) the post-v2.47 UI changes (12-hour clock, WiFi menu removal) are ruled out — they don't exist in v2.47 yet it resets. Still open in v2.47: WiFi-induced Nickel tamper-detect (entry reachable), the install package layout, the boot kill/exec sequence. Do NOT claim "anything we changed since v2.47" is ruled out — v2.47 has the boot-grace skip and the WiFi entry, both of which were suspect in later sessions. Verify by `git show v2.47-sangala:<path>` rather than assuming what v2.47 contains.
