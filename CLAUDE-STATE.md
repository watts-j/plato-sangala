...
47. **The ARM cross-toolchain is self-hosted on this repo because Linaro killed its download host — and the toolchain version is load-bearing, do NOT bump it.** (2026-06-09.) `build-and-release.yml` cross-compiles for the Kobo using the **Linaro 4.9.4-2017.01 `arm-linux-gnueabihf` (hard-float)** toolchain. It is pinned to that exact version on purpose: glibc is backward- but **not** forward-compatible, the Kobo runs an old glibc (~2.19–2.23 vintage), and a newer toolchain (newer Linaro, Arm GNU, or Ubuntu's `apt gcc-arm-linux-gnueabihf`) links the binary against a newer glibc whose `GLIBC_*` symbol versions the device's dynamic linker can't resolve — so Plato would **fail to launch on every device** (`version 'GLIBC_2.xx' not found`), discovered only after deploy. The prebuilt thirdparty `.so` libs (`build.sh fast` → `download.sh`) were built against this same toolchain, so mixing is a second compat hazard. **Failure that forced this (v2.54, v2.55 builds):** the toolchain step did `wget` from `releases.linaro.org` and got exit 4 (network failure / the host now returns **403** for these legacy binaries — Linaro EOL'd its standalone GNU binary toolchain program and handed publishing to Arm). v2.49–v2.52 had built fine **only because the GitHub Actions cache (`actions/cache`, key `linaro-gcc-4.9.4-2017.01`) still held the toolchain**; that cache evicts after ~7 days unused, and the first build after eviction hit the dead URL. The cache was masking a single point of failure on a third-party host. **Fix:** self-host the exact tarball as a **hash-pinned release asset on this repo** and make the cache a mere optimization:
    - **Asset:** the `build-deps` release on `watts-j/plato-sangala`. The tag `build-deps` deliberately does **not** match the workflow's `v*` trigger, so creating/updating it never starts a build.
    - **URL** (in `build-and-release.yml` `env.TOOLCHAIN_URL`): `https://github.com/watts-j/plato-sangala/releases/download/build-deps/gcc-linaro-4.9.4-2017.01-x86_64_arm-linux-gnueabihf.tar.xz`
    - **SHA-256** (`env.TOOLCHAIN_SHA256`, verified fail-closed): `22914118fd963f953824b58107015c6953b5bbdccbdcf25ad9fd9a2f9f11ac07`. This is the original tarball that produced the v2.49–v2.52 builds (found on the user's machine), so it's the authoritative copy. Trust is anchored to the hash, not the host — a corrupt/tampered/changed download fails the build. The workflow also retries the download and errors clearly if the hash is left unset.
    - **Cache is now optional:** an evicted cache just re-downloads from the durable asset; it can no longer break the build.
    - **Re-provisioning runbook** (if the asset is ever lost): obtain the exact file (the original; or a Wayback Machine snapshot of the old Linaro URL; or Arm's "Legacy Linaro GNU Toolchains" archive — `releases.linaro.org` 403s datacenter/CI IPs but may serve a residential browser), verify it (`arm-linux-gnueabihf-gcc --version` must report `4.9.4 ... Linaro GCC 4.9-2017.01`, and/or cross-check the sha256 across two sources), compute `Get-FileHash -Algorithm SHA256` (Windows) / `sha256sum`, upload to the `build-deps` release **with the exact filename**, and set `TOOLCHAIN_SHA256`. The web proxy/integration can't push tags, delete refs, dispatch workflows, or upload release assets (all 403) — release/asset creation and tag pushes are user-side (Lesson #45).

48. **Dictionary lookups returned wrong definitions because the source `.syn` shadows real words — it's bad source data, faithfully converted, not a Plato or conversion-logic bug.** (2026-06-09, v2.56.) Symptom: "book" returned *bake*'s definition, "cook" surfaced the proper noun, "literature" was fine. The reader-dict.com StarDict source ships a `.syn` synonym index (596,917 entries); ~22,902 of them map a word that **already has its own headword** to an unrelated entry (`book`→`bake`). `convert-dictionary.sh` faithfully turns each `.syn` entry into a dictd index entry, so the bogus `book→bake` entry shadows the real word and is returned first. **Confirmed by reproducing the lookup in Python against the StarDict source** (`.idx` = `word\0`+u32 offset+u32 size; `.dict.dz` = gzip; `.syn` = `word\0`+u32 index-into-idx): the device's spurious 1392-byte "book" entry == `bake` (1391 B). The index itself is fine — byte-sorted, clean headwords, Plato's `binary_search` lands correctly (verified neighbor context on-device). **Fix:** filter the shipped `.syn` to keep a synonym only when its word is **not** already a main headword (`contrib/filter-synonyms.py`). Drops the 22,902 shadowing entries; keeps 574,015 valuable inflection-only lookups (`mice`→`mouse`, `geese`→`goose`). No word becomes unlookable — words with their own (substantive) entry resolve to it. Spot-checked 100 common words incl. homographs (lead/bass/bow/tear/wind/bat): **0 shadows, 0 unlookable**. Data-only; ships in `-install.tar.gz`; the device re-runs its ~3-min conversion on next install, so existing devices aren't fixed until reinstalled. **Diagnostic trap avoided:** `sangala/dictionaries-converted/` is a STALE dev artifact with mangled headwords — NOT what the device produces. The device builds its own index from `sangala/dictionaries/` (StarDict source) on first boot; trust the source + device files, not `dictionaries-converted/`. **Plato internals worth knowing** (`crates/core/src/dictionary/`): `find` uses byte-wise `binary_search_by_key`, so the index must be byte-sorted; the corrective `normalize()` is an O(n²) insertion sort and is skipped when a headword contains "dictfmt" (`has_dictfmt`), so correctness relies on the converted index already being byte-sorted (it is). Lookup lowercases + alphanumeric-filters the query per the `00-database-allchars`/`-case-sensitive` metadata.
