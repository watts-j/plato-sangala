...
**Confirmed: bug is structural to the kill-Nickel-on-every-boot architecture.** v2.32 and v2.47 both fail; the difference between them is irrelevant. Incremental fixes won't help.

1. **Search MobileRead (Kobo Developer's Corner) for Clara BW + Plato + factory reset / recovery, and for whatever Kobo firmware uses as a boot-failure counter.** Other Plato-on-Kobo deployments have to deal with this — find out what they do. Specifically look at how KFMon-based Plato installs avoid the issue. This is the cheapest, most informative next step.
2. **Snapshot the device after each boot of a multi-cycle run** (boot 1 working, boot 2 working, boot 3 working, boot 4 reset). Diff `.kobo/`, `Settings.toml`, `Kobo eReader.conf`, and `/var/log` (if accessible) to identify the file Kobo is incrementing. Snapshot tool from Lesson #34 should be committed to `tools/`.
3. **Investigate restoring KFMon-based launching.** The upstream Plato community uses KFMon — it doesn't kill Nickel, it adds a Kobo Reader collection icon that launches Plato as a separate process. v2.28's KFMon experiment failed because we kept `plato-autostart.sh` alongside it (two competing launchers). Properly: remove `plato-autostart.sh` AND `on-animator.sh`, reintroduce v2.3's KFMon binaries, configure a KFMon icon that runs `plato.sh`. Substantial work, but matches what the rest of the Plato community does. Lesson #13.
4. **Or accept Nickel and rebuild Sangala UX inside Nickel** (custom collections, custom KEPUBs with chapter overrides) — much less Plato-like but eliminates the kill-Nickel architecture entirely.
5. **Do NOT do the v2.24 rewrite or any other "rebuild on an old base"** — the old bases all fail too.
6. **The v2.49 production batch (~30 devices) is at risk.** Field deployments may start factory-resetting once users cumulatively power-cycle enough. If field reports start coming in, the answer is the architectural fix, not an incremental patch.
