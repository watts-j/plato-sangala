...
Last updated: 2026-05-25 (cable swap confirmed; factory-reset is a separate device-level bug; v2.53 tagged with failed revert — see warnings below)
...
- **No version is currently confirmed stable.** As of 2026-05-25, the test device (Clara BW, serial N365594030627, firmware 4.42.23291) factory-resets on the ~2nd power cycle regardless of which version is installed, even after a USB cable swap (see Lesson #35). The cable swap DID resolve install-time frozen-dots failures (Lesson #30 pattern confirmed), but the post-install reboot reset is a separate problem. Current highest hypothesis: the test device has degraded eMMC or a hardware fault from many install cycles; **needs a second Clara BW to determine if device-specific or software bug.** The v2.49 production batch of ~30 devices is presumed at-risk (untested under multiple power cycles). See Session-End Handoff (2026-05-23) for full history.
...
### What was NOT tried (highest-priority untested hypothesis)

**Test a different Clara BW device.** The cable swap (2026-05-25) resolved install-time frozen-dots — confirming Lesson #30 — but the test device still factory-reset on the 2nd power cycle after a clean install. The test device (N365594030627) has been factory-reset and reinstalled dozens of times across the development history, making eMMC degradation or hardware fault the most plausible remaining hypothesis. **A second Clara BW device is the next diagnostic step.** If it installs and survives 10+ power cycles with v2.52, the test device is the problem. If it also resets, we have a reproducible software bug.

### Ruled out (don't re-investigate without new evidence)

- v2.44 boot-grace skip — reverted, still resets.
- GUI installer specifically — CLI also resets on this device.
- Any change we made since v2.24 — v2.24 manual drag-drop install also fails.
- Kobo firmware OTA — user confirms `4.42.23291` has been the firmware throughout this device's life.
- "Enable WiFi" burger menu entry — devices factory-reset without WiFi being touched.
- **USB cable** — cable swap resolved install-time frozen dots (Lesson #30 confirmed), but the same device still factory-reset on the 2nd reboot with the new cable. Cable is not the cause of the factory-reset bug.

### Open hypotheses (ranked, best to worst)

1. **Test device eMMC degradation or hardware fault** — this device has been factory-reset and reinstalled far more than any production device; write-cycle wear or a latent hardware issue is plausible. Cheapest test: install on a second, less-cycled Clara BW and power-cycle 10+ times.
2. **Different USB port on the host** — untested, lower plausibility now that cable swap confirmed.
3. **Something in Kobo firmware 4.42.23291's boot-integrity / tamper-detect path** that wasn't present in earlier firmwares — can't test without an older firmware image.
4. **Something we haven't thought of.** External resources (Kobo firmware documentation, MobileRead forums) might shed light.
...
1. **Install v2.52 on a DIFFERENT Clara BW device** (not N365594030627). Power-cycle 10+ times. If it survives, the test device has a hardware fault and v2.52 is effectively stable.
2. **If the second device also resets**: the bug is reproducible across hardware; escalate to code-level investigation. External resources (MobileRead forums, Kobo firmware changelogs) may help.
3. **v2.53 is already tagged** with just the failed boot-grace revert — the mistake the original handoff tried to prevent. Don't reuse the v2.53 number; next real fix tags v2.54. Consider deleting or warning-stamping the v2.53 release before another batch deployment.
4. **Do NOT do the v2.24 rewrite** — it doesn't fix anything the investigation hasn't already disproven.
5. The v2.49 production batch (~30 devices) is presumed at-risk but no reported resets from field yet. If the second-device test passes, the batch is likely fine.
...
- **Factory reset on power-cycle (TOP PRIORITY, UNRESOLVED).** Test device factory-resets on ~2nd power cycle regardless of installed version (v2.24 manual drag-drop through v2.53 CLI). Four hypotheses tested and disproven (see Lesson #32 and #35). **Most likely remaining cause: test device hardware (eMMC wear or hardware fault).** Next step: test a different Clara BW unit with v2.52 and 10+ power cycles. Do NOT tag any version as stable until a different device survives that protocol.
...
32. **Kobo Clara BW devices factory-reset on boot after installing this build; root cause not yet found.** Symptom: device crashes during boot (often the ~2nd–3rd power cycle), enters recovery mode, factory-resets to Kobo's OOBE wizard. Sometimes installs land cleanly then fail on later boots; sometimes the first post-install boot itself fails with frozen non-animated loading dots after the "updating" screen. **Four explicit hypotheses have been tested and disproven on the test device** (Clara BW, serial `N365594030627`, firmware `4.42.23291` — unchanged across all tests): (a) ~~GUI installer is the cause~~ (v2.49 CLI install also resets); (b) ~~Boot-grace skip in v2.44's commit `2566fc7` is the cause~~ (reverted in commit `ebcf55c` on branch; device still resets); (c) ~~Anything we added since v2.24 is the cause~~ (v2.24 itself installed via plain manual drag-drop and Windows' Safely Remove Hardware produces the same frozen-dots symptom on this device); (d) ~~Flaky USB cable causes the factory reset~~ (cable swap with a new cable resolved install-time frozen dots per Lesson #30, but device still factory-reset on the 2nd power cycle with the new cable — see Lesson #35). **Highest remaining hypothesis: test device hardware.** N365594030627 has been factory-reset and reinstalled far more than any production device; eMMC write-cycle wear or a latent hardware fault is now the most plausible explanation. Cannot get device-side logs after a reset (user partition is wiped); the host-side `install-sangala.log` consistently looks clean. **Next step: try a different Clara BW device.** Do not tag any v2.5x with a "fix" for this until at least one device survives 10+ power cycles.
...
35. **USB cable swap separates two distinct failure modes.** (2026-05-25) On the test device (N365594030627), a cable swap resolved install-time frozen-dots (Lesson #30 pattern confirmed), but the device still factory-reset on the 2nd power cycle after a clean install with the new cable. This proves the frozen-dots failure and the factory-reset-on-reboot failure are **two separate bugs**, not one. The cable only fixes the first. The factory-reset bug's remaining hypothesis is test-device hardware (eMMC wear from many install/reset cycles). Before writing any more code, try the install on a different Clara BW that hasn't been repeatedly reset.
