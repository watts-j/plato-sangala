...
Last updated: 2026-05-25 (v2.47 resets new Clara BW via both CLI and manual install; WiFi never toggled; bug is in installed contents, not install path; reboot count to reset varies 2–4)
...
- **No version is currently confirmed stable.** As of 2026-05-25, factory reset has now been reproduced TWICE on the new device with v2.47: (a) via CLI installer, reset on 2nd power cycle; (b) via fully manual install (drag-drop + Safely Remove Hardware, no installer code at all), reset on 4th slow reboot. Together these rule out: the installer code path entirely, the eject sequence, the host-side flush logic, cable, eMMC wear, post-v2.47 UI changes (12-hour clock, WiFi removal), the GUI installer. WiFi was never toggled. **The bug is in WHAT is installed, not HOW it is installed**, and the reboot-count-to-reset varies (2 vs 4), suggesting cumulative state buildup across boots that eventually trips Kobo's recovery. The v2.49 production batch of ~30 devices is presumed at-risk under multi-power-cycle (untested). See Session-End Handoff for the investigation history.
...
### Update 2026-05-25: factory reset reproduces with v2.47 via CLI AND manual install on a new device; WiFi confirmed never toggled
...
1. **CLI installer** — device reset on 2nd power cycle.
2. **Manual install** (drag-drop + Safely Remove Hardware, no installer code at all) — device reset on 4th slow reboot.

WiFi was never toggled in either run. v2.47 verified contents (from `git show v2.47-sangala`): v2.44 boot-grace skip present in `plato-autostart.sh`; Enable WiFi entry still in `toggle_main_menu` (but not used); 24-hour clock; install zip is CLI-only.

- **eMMC wear ruled out** — new device, no prior install/reset history.
- **Test-device hardware fault ruled out** — different device, same symptom.
...
- **Eject sequence ruled out** — manual install bypasses eject IOCTLs entirely.
- **Host-side `Flush-Drive` ruled out** — manual install doesn't call it.
- **Post-v2.47 UI changes (12-hour clock, WiFi menu removal) ruled out** — v2.47 has none of them.
- **WiFi tamper-detect ruled out** — never toggled in the new-device runs.

**Reboot count to reset varies (2 vs 4)**, which suggests cumulative state buildup across boots — something written to `.kobo/` (or to Settings.toml / Kobo eReader.conf) by Nickel or Plato on each boot eventually reaches a state Kobo's recovery interprets as a broken system.
...
- Any change we added since v2.24 — v2.24 manual drag-drop also fails.
...
- 12-hour clock / removed date / removed WiFi menu — none of those exist in v2.47 yet it resets.
- **WiFi tamper-detect** — WiFi was never toggled in the new-device runs.

### Open hypotheses (ranked, best to worst)

1. **Cumulative state across boots.** Reboot count to reset varies (2 vs 4) — strongly suggests something written on each boot accumulates until Kobo's recovery trips. Diff the device's `.kobo/`, `Settings.toml`, and `Kobo eReader.conf` between boots (use snapshot tool from Lesson #34, which still needs to live in `tools/`).
2. **`plato-autostart.sh` killing Nickel mid-init.** v2.47 (and v2.44+) skips the 5s grace on subsequent boots — kills Nickel as soon as it's up. If Nickel was mid-write to `KoboReader.sqlite` or `version` when killed, partial writes could accumulate across boots until Kobo's integrity check fails. Hypothesis is testable by comparing v2.47 to v2.32/v2.30 (which kept the grace on every boot).
3. **Something v2.47 changed vs v2.32/v2.30 that we haven't traced.** v2.30 and v2.32 were previously claimed stable (install-and-boot-once) — if they survive multi-power-cycle on the same new device, the bug is in something added between v2.32 and v2.47.
4. **Something we haven't thought of.** MobileRead, Kobo firmware notes for 4.42.x.
...
The factory reset is reproducible across hardware (new Clara BW) AND across install methods (CLI installer, manual drag-drop). WiFi was confirmed never toggled. The bug is in what gets installed onto the device, not in the install process.

1. **Test v2.32 on a fresh device with the same multi-power-cycle protocol.** v2.32 was previously declared stable and has different `plato-autostart.sh` semantics (kept the 5s grace on every boot, not just first-boot). If v2.32 survives → bug was introduced between v2.32 and v2.47, bisect from there. If v2.32 also resets → bug has been latent the whole time, never multi-power-cycle-tested.
2. **Snapshot the device's state between each boot until the reset happens** (Lesson #34). With reset happening on the 2nd vs 4th boot, snapshot after boot 1, boot 2, boot 3, etc. and diff to see what's drifting. Snapshot tool from Lesson #34 still needs to live in `tools/`.
3. **Diff `plato-autostart.sh`, `plato.sh`, and KoboRoot.tgz between v2.30/v2.32 and v2.47.** Pay attention to kill order for `nickel`/`on-animator` and to whether anything was added to KoboRoot.tgz.
4. **Search MobileRead forums for Clara BW + firmware 4.42 + recovery / factory reset behavior.** May be a documented community problem.
5. **Do NOT do the v2.24 rewrite** — v2.24 manual drag-drop also fails.
6. The v2.49 production batch (~30 devices) is at the same risk level as v2.47 since v2.47 itself now resets a fresh device.
...
32. **Kobo Clara BW devices factory-reset on boot after installing this build; root cause not yet found.** Symptom: device factory-resets on the 2nd–4th power cycle, enters recovery mode, lands at Kobo's OOBE wizard. **Hypotheses tested and disproven** as of 2026-05-25: (a) ~~GUI installer is the cause~~ (v2.49 CLI install resets the test device; v2.47 CLI-only also resets a new device; v2.47 manual drag-drop install with no installer code also resets); (b) ~~Anything we added since v2.24 is the cause~~ (v2.24 manual drag-drop also fails); (c) ~~Flaky USB cable causes the reset~~ (cable swap resolved install-time frozen dots — Lesson #30 — but device still resets); (d) ~~Test-device-specific hardware fault or eMMC wear on N365594030627~~ (a brand-new Clara BW that had never been factory-reset before also resets — see Lesson #35); (e) ~~Post-v2.47 UI changes are the cause~~ (v2.47 has neither the 12-hour clock nor the WiFi-menu removal, yet resets); (f) ~~WiFi-induced Nickel tamper-detect~~ (WiFi was never toggled in the new-device runs); (g) ~~Any host-side install logic — CLI installer, GUI installer, eject sequence, FlushFileBuffers~~ (a fully manual drag-drop + Safely Remove Hardware install of v2.47 also resets, this time on the 4th slow reboot). **The bug is in what gets installed, not how it's installed.** Reboot count to reset varies (2 vs 4) — strongly suggests cumulative state buildup across boots. Cannot get device-side logs after a reset (user partition wiped). Next-step direction: test v2.32 on a fresh device (its `plato-autostart.sh` kept the 5s grace on every boot, unlike v2.47's no-grace-on-subsequent); snapshot device state between each boot and diff for drift; diff `plato-autostart.sh`/`plato.sh`/KoboRoot.tgz between v2.30/v2.32 and v2.47; search MobileRead for Clara BW + firmware 4.42 recovery behavior. Do not tag any v2.5x as stable until at least one device survives 10+ power cycles.
33. **Don't declare a version "stable" from a successful install alone — power-cycle 10+ times first.** v2.49's production batch was marked stable based on ~30 devices "installing cleanly." The user hadn't tested any of them under repeated power-cycling. v2.49 is now confirmed (2026-05-23) to factory-reset on ~3rd power cycle on the test device, and the production-batch devices are presumed at-risk. Same for v2.48, v2.32, v2.30 — all marked stable based on install-and-boot-once tests. The 2026-05-23 investigation exposed that an install-and-boot-once test is not sufficient evidence of stability. Stability claims in CLAUDE-STATE prior to 2026-05-23 should be treated with skepticism.
...
35. **USB cable swap separates two distinct failure modes; reset reproduces on fresh hardware with v2.47, via both CLI and fully manual install; reboot count varies (2 vs 4).** (2026-05-25) A cable swap resolved install-time frozen-dots (Lesson #30 confirmed). Then v2.47 was installed twice on a **brand-new Clara BW** (never previously factory-reset) with the good cable: once via CLI installer (reset on 2nd power cycle), once via fully manual drag-drop + Safely Remove Hardware (reset on 4th slow reboot). WiFi was never toggled in either run. v2.47 verified contents: v2.44 boot-grace skip present; Enable WiFi entry still in `toggle_main_menu`; 24-hour clock; CLI-only install zip. Implications: (a) frozen-dots-at-install and factory-reset-on-reboot are two separate bugs; (b) eMMC wear / device-specific fault on N365594030627 ruled out; (c) all install paths ruled out — CLI, GUI, manual all reset; (d) the post-v2.47 UI changes ruled out — they don't exist in v2.47; (e) WiFi tamper-detect ruled out (never toggled); (f) variable reboot count (2 vs 4) suggests cumulative state buildup across boots, not a fixed trigger on a specific boot. The bug is in what gets installed onto the device, not in any host-side process. Verify v2.47 contents by `git show v2.47-sangala:<path>` rather than asserting them from CLAUDE-STATE prose.
