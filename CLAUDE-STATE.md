...
- **No version is currently confirmed stable.** As of 2026-05-25, factory reset on the ~2nd power cycle has now been reproduced on a **new, never-previously-reset Clara BW** running **v2.47** (the prior "stable" baseline) with a known-good USB cable. This rules out: (a) cable as the reset cause (cable swap DID fix install-time frozen dots — Lesson #30 confirmed — but not the reset); (b) eMMC wear / hardware fault on test device N365594030627; (c) anything we changed after v2.47 (GUI installer, boot-grace skip, WiFi menu entry, clock UI, etc.). The bug exists in v2.47 itself and is reproducible across hardware. The v2.49 production batch of ~30 devices is presumed at-risk under multi-power-cycle (untested). See Session-End Handoff for the investigation history.
...
After the cable swap (which DID resolve install-time frozen dots — Lesson #30 confirmed), a clean v2.47 install on a **brand-new Clara BW that had never been factory-reset before** factory-reset on the 2nd power cycle. This is the critical new data point:
...
- **Anything after v2.47 ruled out** — v2.47 itself is the version that failed.
- **Cable ruled out for the reset** — new cable, still resets.

### Ruled out (don't re-investigate without new evidence)

- v2.44 boot-grace skip — reverted, still resets.
- GUI installer specifically — CLI also resets.
- Any change we made since v2.24 — v2.24 manual drag-drop install also fails; v2.47 (the prior stable baseline) also fails on a new device.
- Kobo firmware OTA changing under us — user confirms `4.42.23291` has been the firmware throughout.
- "Enable WiFi" burger menu entry — devices factory-reset without WiFi being touched.
...
1. **Something in the install package or `plato-autostart.sh` interacts badly with Kobo firmware 4.42.23291's boot-integrity / tamper-detect path.** v2.47's autostart kills `nickel` and `on-animator` and execs `plato.sh`; if Kobo's anti-tamper sees an unexpected process state across reboots, it may trip recovery. Diff `plato-autostart.sh` and `KoboRoot.tgz` between the earlier-"stable" eras (v2.30/v2.32) and v2.47 to see if the kill/exec sequence shifted.
2. **Kobo firmware 4.42.23291 changed boot behavior** relative to firmwares that were current when v2.30/v2.32 were declared stable. We can't roll the firmware back to test directly, but MobileRead forums likely have notes on 4.42.x changes.
3. **Different USB port on the host** — still untested. Lower plausibility, but cheap.
4. **Something we haven't thought of.** External resources (Kobo firmware documentation, MobileRead forums) may help.
...
The factory reset is reproducible across hardware (new Clara BW with v2.47). The bug exists in v2.47 itself or its install layout, not in anything specific to the test device or a flaky cable.

1. **Diff `plato-autostart.sh`, `plato.sh`, and the KoboRoot.tgz contents between v2.30/v2.32 (eras user remembers as actually stable across reboots) and v2.47.** The factory-reset symptom on the 2nd power cycle suggests the boot path is doing something Kobo's recovery detects as "broken system." Look specifically at the kill order for `nickel` and `on-animator` and at any new files inside KoboRoot.tgz.
2. **Pull a device-state snapshot AFTER a successful 1st boot, BEFORE the 2nd power cycle** that triggers the reset. If `.kobo/` has gained anything new, or `Settings.toml`/`Kobo eReader.conf` differs from what was shipped, that's a clue. Snapshot tool needs to live in `tools/` (Lesson #34).
3. **Search MobileRead forums for Clara BW + firmware 4.42 + recovery / factory reset behavior.** This may be a documented community problem.
4. **v2.53 is already tagged** with just the failed boot-grace revert — the mistake the original handoff tried to prevent. Don't reuse v2.53; next real fix tags v2.54. Consider deleting or warning-stamping the v2.53 release.
5. **Do NOT do the v2.24 rewrite** — v2.24 manual drag-drop also fails, so rewriting against it does not fix this bug.
...
32. **Kobo Clara BW devices factory-reset on boot after installing this build; root cause not yet found.** Symptom: device factory-resets on the ~2nd power cycle, enters recovery mode, lands at Kobo's OOBE wizard. **Five hypotheses tested and disproven** as of 2026-05-25: (a) ~~GUI installer is the cause~~ (v2.49 CLI install also resets); (b) ~~Boot-grace skip in v2.44's commit `2566fc7` is the cause~~ (reverted in commit `ebcf55c`; still resets); (c) ~~Anything we added since v2.24 is the cause~~ (v2.24 manual drag-drop also fails); (d) ~~Flaky USB cable causes the reset~~ (cable swap resolved install-time frozen dots — Lesson #30 — but device still reset on 2nd power cycle with the new cable); (e) ~~Test-device-specific hardware fault or eMMC wear on N365594030627~~ (a brand-new Clara BW that had never been factory-reset before, running v2.47 with a good cable, also factory-reset on the 2nd power cycle — see Lesson #35). **The bug is reproducible across hardware and is present in v2.47 itself, the previously claimed stable baseline.** Cannot get device-side logs after a reset (user partition wiped). Next-step direction: diff `plato-autostart.sh`/`plato.sh`/KoboRoot.tgz between v2.30/v2.32 and v2.47, pull a device snapshot between 1st boot success and 2nd-boot reset, search MobileRead for Clara BW + firmware 4.42.23291 recovery behavior. Do not tag any v2.5x as stable until at least one device survives 10+ power cycles.
...
35. **USB cable swap separates two distinct failure modes; the reset reproduces on fresh hardware with v2.47.** (2026-05-25) A cable swap resolved install-time frozen-dots (Lesson #30 pattern confirmed). Then a clean v2.47 install on a **brand-new Clara BW** (never previously factory-reset) with the good cable still factory-reset on the 2nd power cycle. Implications: (a) frozen-dots-at-install and factory-reset-on-reboot are two separate bugs — the cable only fixes the first; (b) eMMC wear and device-specific hardware fault on the original test device N365594030627 are ruled out — a fresh device shows the same behavior; (c) the bug is in v2.47 itself, or in the install package layout, or in how the bootstrap interacts with Kobo firmware 4.42.23291. Investigation should now focus on diffing what changed between an actually-stable era (likely v2.30/v2.32, since user remembers those surviving multi-power-cycle in earlier deployments) and v2.47 — specifically `plato-autostart.sh`, `plato.sh`, and the KoboRoot.tgz contents — and on whether anything in `.kobo/` changes between the 1st (working) boot and the 2nd (reset-triggering) boot.
