...
Last updated: 2026-05-25 (multi-power-cycle protocol confirms the bug is structural — v2.32 also factory-resets at 4 cycles. Nickel cannot replicate Sangala's UX. KFMon-based launching is the lead architectural candidate. Snapshot tools added under tools/.)
...
- **No version is currently confirmed stable.** Update 2026-05-25: the multi-power-cycle protocol (install on fresh device, open a book, dictionary lookup, power off, wait 20–30s, power on, repeat) factory-resets v2.32 on the 4th cycle and v2.47 on cycle 2 (CLI install) or cycle 4 (manual drag-drop install) on previously-never-reset Clara BW devices. WiFi never toggled. **The bug has been latent in the entire project**; the v2.45/v2.46 "stable" device may simply not have been power-cycled enough to hit it. The dictionary-removal hypothesis from the 2026-05-23 handoff (#2) is now demoted because v2.32's `convert-dictionary.sh` predates the `cp` fallback and would silently fail on FAT just like v2.45/v2.46 — so v2.32 also has no converted dictionary files yet still resets. Confirmed: bug is structural to the kill-Nickel-on-every-boot architecture, not in dictionary conversion specifically. See Session-End Handoff (2026-05-25) for full evidence.
...
- **Factory reset on power-cycle (TOP PRIORITY, UNRESOLVED).** Confirmed on Clara BW for v2.32, v2.47, v2.48, v2.48.1, and the discarded v2.49-v2.53 line. **v2.32 also resets at 4 cycles** (2026-05-25, fresh device, proper protocol — open book/dictionary lookup/power off/wait 20-30s/repeat). The bug has been latent in the entire project; no version is safe. The v2.46→v2.47 dictionary hypothesis is demoted (v2.32 predates it and still resets). Leading hypothesis is now structural: Kobo bootloader watchdog counts unhealthy boots until it triggers recovery; `plato-autostart.sh` kills Nickel before any "healthy" marker is written. Next step: snapshot protocol via `tools/snapshot-device.ps1` to identify the marker; if found, manage it from `plato-autostart.sh`. If not, switch to KFMon-based launching (Plato community standard, doesn't kill Nickel). Lesson #36: do NOT propose Nickel as an alternative — it cannot replicate Sangala's UX.
