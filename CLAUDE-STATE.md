...
- **Home landing page**: Implemented in `Shelf::update`'s empty-state branch (no overlay). When the active library is intrinsically empty (no books and no subdirs anywhere — verified via `library.list(library.home, None, false)`) and both `home-image` and `welcome-name` are set in `[home]`, Shelf pushes an `Image` view (top 2/3 of its rect, scaled to fit via the document framework) and a centered `Label` ("Welcome, {welcome-name}!") in the bottom 1/3 instead of the white filler. Falls back to white filler if either setting is missing, the image fails to load, or the library has any content (so a search filter that yields zero matches in a populated library doesn't trigger the welcome screen). The dead `HomeImage` overlay view and its `remove_home_image` plumbing were deleted.
