...
Last updated: 2026-05-25 (v2.32 ALSO factory-resets after 4 power cycles on a fresh device — bug has been latent in the entire project, ~30 production devices presumed at-risk)
...
- **No version is currently confirmed stable.** As of 2026-05-25, factory reset on the ~2nd–4th power cycle has been reproduced on a fresh Clara BW with **v2.47** (CLI install, reset on cycle 2; manual drag-drop install, reset on cycle 4) AND with **v2.32** (manual install, normal usage between cycles — open a book, check a definition, power off, wait 20–30s, repeat — reset on cycle 4). v2.32 was the earliest version we ever called "stable." **The bug has been latent in the project the entire time**; it wasn't caught earlier because none of the original "stable" verifications power-cycled the device more than 2–3 times in close succession. The ~30 production v2.49 devices are presumed at-risk for the same reason. Cable, eMMC wear, installer (CLI, GUI, or manual), eject sequence, host-side flush, WiFi tamper-detect, and every UI change post-v2.32 are all ruled out. The reset is triggered by something fundamental to the Plato + plato-autostart.sh + KoboRoot.tgz architecture as it interacts with Kobo firmware 4.42.23291. See Session-End Handoff for the investigation history.
- **v2.53-sangala — DO NOT INSTALL.** Pre-release tagged 2026-05-23 at commit `104f326`. Contents over v2.52: only the boot-grace revert (`ebcf55c`) plus a CLAUDE-STATE update — **no fix for the factory-reset bug.** The revert was device-tested and did NOT stop the reset. The tag was created in spite of the handoff explicitly saying not to ship this; the release body is the standard auto-generated template with no warning. Anyone installing v2.53 expecting a fix will still factory-reset on the test device. Discovered (and warned about here) on 2026-05-23. Consider deleting the v2.53 release + tag or editing the release body to prepend a "DO NOT INSTALL" warning.
...
## Session-End Handoff (2026-05-23, factory-reset investigation, no fix found)

### Status

The factory-reset bug is **active and unfixed**. Multiple installs on the same Clara BW test device through this session, with at least three observed symptom variations:

- Boot to Plato, work briefly, factory-reset on ~3rd power cycle (recovery mode → OOBE wizard).
- Phase 2 eject completes per `install-sangala.log`, device then boots straight to frozen non-animated loading dots.
- "Updating" screen shown, then frozen loading dots.

The symptoms appear regardless of which installer is used (manual drag-drop on v2.24, CLI .ps1 on v2.49/v2.53, GUI .exe on v2.49) and regardless of which version is installed. **Same device, same firmware (4.42.23291), same USB cable, same host computer throughout.** That rules out the hypotheses we acted on earlier in the session.

### What was tried (and what each told us)

1. **Dictionary-in-update-package fix (v2.50, shipped).** Lesson #31. Real bug, real fix. **Not the cause of resets** — fixed a different bug (wrong-offset dictionary lookups).
2. **Drop GUI `.exe` from release zip (v2.51, shipped).** Acted on a "GUI installer is the cause" hypothesis based on the v2.49 production batch using GUI installs. **Not the fix** — a v2.49 CLI install also factory-reset on this device (2026-05-23). Theory disproven.
3. **Remove "Enable WiFi" from burger menu (v2.51, shipped).** Eliminated one possible trigger (WiFi → Nickel OTA tamper-detect). **Not the cause** — devices factory-reset without ever attempting WiFi.
4. **12-hour clock + simplified menu (v2.52, shipped).** UX improvement only. Not bug-related.
5. **Revert `2566fc7` (the v2.44 boot-grace skip) (v2.53 prepared, NOT tagged).** Strongest hypothesis at the time: kill-Nickel-immediately on subsequent boots was the only on-device script change between user's "v2.32 was stable" anchor and the resetting v2.49. Branch has commit `ebcf55c` reverting it. Device-tested: **still resets on ~3rd power cycle.** Theory disproven.
6. **Manual drag-drop install of v2.24 (no installer code involved).** v2.24 was the user's mental anchor for "definitely stable." Result on the test device: **frozen loading dots after the updating screen.** v2.24 fails on this device with the same symptom. The bug pre-dates the entire customization arc, or is hardware-specific to this device.

### Update 2026-05-25: factory reset reproduces with v2.47 via CLI AND manual install AND with v2.32 — bug has been latent all along

After the cable swap (which DID resolve install-time frozen dots — Lesson #30 confirmed), v2.47 was installed on a **brand-new Clara BW that had never been factory-reset before** TWO ways:

1. **v2.47 CLI installer** — device reset on 2nd power cycle.
2. **v2.47 manual install** (drag-drop + Safely Remove Hardware, no installer code at all) — device reset on 4th slow reboot.

Then **v2.32** was installed manually on another fresh device. Normal usage between cycles (open a book, check a dictionary definition, power off, wait 20–30s, repeat). Device factory-reset on the 4th cycle.

WiFi was never toggled in any of the runs. v2.47 and v2.32 verified contents read directly from the tags (`git show vX.Y-sangala`).

**This means the bug has been latent in the entire project.** v2.32 was the earliest version we ever marked stable; it also fails under multi-power-cycle. The reason no one caught it earlier is that none of the original "stable" verifications power-cycled the device more than 2–3 times in close succession. Lesson #33 had already flagged this risk; the v2.32 test now confirms it.

- **eMMC wear ruled out** — new device, no prior install/reset history.
- **Test-device hardware fault ruled out** — multiple devices, same symptom.
- **Cable ruled out for the reset** — new cable.
- **GUI installer ruled out** — v2.47 zip is CLI-only.
- **CLI installer ruled out** — manual install also fails.
- **Eject sequence and `Flush-Drive` ruled out** — manual install bypasses them.
- **Post-v2.32 changes ruled out** — v2.32 itself fails.
- **WiFi tamper-detect ruled out** — never toggled.
- **The v2.44 boot-grace skip ruled out** — v2.32 predates it and still fails.

**Reboot count to reset varies (2–4), and v2.32 also resets — the bug is structural, not regression-introduced.** It's something in the Plato + `plato-autostart.sh` + minimal KoboRoot.tgz pattern as it interacts with Kobo firmware 4.42.23291. Most likely candidate: Kobo's recovery / watchdog noticing repeated Nickel kills (or absence of Nickel-managed state writes) and concluding the system is broken.

### Ruled out (don't re-investigate without new evidence)

- GUI installer / CLI installer / manual installer — all three install paths produce the reset.
- Eject sequence / Flush-Drive / any host-side flow — manual install bypasses them.
- Any change we added since v2.32 — v2.32 also fails.
- Kobo firmware OTA changing under us — firmware `4.42.23291` has been steady.
- **USB cable** — cable swap resolved install-time frozen dots (Lesson #30 confirmed) but not the reset.
- **eMMC wear / test-device hardware fault** — reset reproduces on a new, never-reset Clara BW.
- 12-hour clock / removed date / removed WiFi menu / GUI installer / v2.44 boot-grace skip — none exist in v2.32, yet v2.32 resets.
- **WiFi tamper-detect** — WiFi was never toggled in any new-device runs.

### Open hypotheses (ranked, best to worst)

1. **Kobo's recovery / watchdog reacts to Nickel being killed every boot.** Kobo firmware tracks boot health via some mechanism (commonly a boot-attempt counter that resets on a "successful" Nickel session; if N consecutive boots don't look healthy, the bootloader triggers recovery and wipes the user partition). Every boot under our architecture: `plato-autostart.sh` waits for Nickel, then `pkill -f on-animator` and `exec plato.sh` (which then `killall -TERM nickel`). Kobo may not see those as healthy Nickel sessions. Reboot count to reset = 4 in both v2.32 and v2.47 manual installs — consistent with a counter that requires N=3–5 bad boots.
2. **`plato.sh` killing `nickel` violates an invariant Kobo's bootloader checks** (e.g., Nickel must reach a specific state, write a specific marker, or exit cleanly within a window).
3. **The "minimal KoboRoot.tgz" approach** (v2.20+, dropping KFMon) removed something Kobo's recovery needs to see — the v2.3 KFMon-based layout that the upstream Plato community uses doesn't kill Nickel, it lets Nickel run and uses KFMon's icon-launch mechanism. v2.28 tried to re-add KFMon and failed because we kept `plato-autostart.sh` alongside it (Lesson #13). Properly removing `plato-autostart.sh` and reintroducing KFMon would be a substantial architectural change but matches what other Plato-on-Kobo deployments do.
4. **Something we haven't thought of.** MobileRead Plato + Kobo threads.
...
- Kobo Clara BW, serial `N365594030627`, firmware `4.42.23291`, kernel `4.9.77`, model 391.
- Pulled from `.kobo/version` on 2026-05-23 via `Get-Content "C:\Users\jbw3r\Desktop\Snapshots\20260523-055243\.kobo\version"`.
- Plato binary SHA256 on disk after v2.53 install: `CFEA016DD0C847A96EC41042D44B5F9B578AF1DAF70A0A601E502F35D4AF...` (snapshot 20260523-055243).
- One device-side snapshot exists at `C:\Users\jbw3r\Desktop\Snapshots\20260523-055243\` containing `info.log` (Plato's), `dictionary.log`, `Settings.toml`, `.kobo/` contents, and a `manifest.json` of key file hashes/sizes/mtimes. The snippet to produce more such snapshots is in the conversation history under "Pull a snapshot of the device's state to your host" — would be a good thing to commit to the repo as a `tools/` script next session.

### User's proposed next direction (not executed)

On 2026-05-23 the user proposed: roll the codebase back to v2.24 as a clean base and re-apply a specific set of features on top (WelcomeScreen, welcome-name, Title Menu Home, dot-prefixed library folders, restricted burger menu, dictionary improvements, removed clock/date/WiFi/Invert/Reboot). **Not executed** because the v2.24 install test on the same test device produced the same frozen-dots symptom, so the rewrite is unlikely to fix anything before the cable/hardware path is explored. If next session tries the cable swap and v2.24 still fails, the rewrite remains a future option for narrowing down further.

### Recommended first actions for next session (updated 2026-05-25)

**Confirmed: bug is structural to the kill-Nickel-on-every-boot architecture.** v2.32 and v2.47 both fail; the difference between them is irrelevant. Incremental fixes won't help. **Nickel cannot replicate Sangala's UX** (hierarchical taxonomy and welcome-screen-with-name are impossible in Nickel even with NickelMenu/kobopatch — per 2026-05-25 research). Plato is required; the question is how to launch it without tripping Kobo's recovery.

1. **Run the snapshot protocol** (`tools/snapshot-device.ps1` + `tools/diff-snapshots.ps1`, committed 2026-05-25) between every boot of a multi-cycle test on a fresh device. Diff consecutive snapshots to find what's drifting. High-value candidates to watch in `.kobo/Kobo/Kobo eReader.conf`: `sleepOnNextBoot`, `syncOnNextBoot`, `LastFTEStep`. Also watch for new files appearing in `.kobo/` and for the `version` file changing each boot.
2. **Investigate restoring KFMon-based launching** if the snapshot protocol doesn't reveal a writable marker we can manage from `plato-autostart.sh`. KFMon doesn't kill Nickel — it adds a Kobo Reader collection icon that launches Plato as a separate process. v2.28's attempt failed because we kept `plato-autostart.sh` alongside it (two competing launchers — Lesson #13). Properly: remove `plato-autostart.sh` AND the minimal `on-animator.sh` modification, reintroduce v2.3's KFMon binaries, configure a KFMon icon that runs `plato.sh`. Substantial work, but matches the upstream Plato community pattern that doesn't have this problem.
3. **Search MobileRead Kobo Developer's Corner** for the specific boot-failure-counter mechanism in Kobo firmware 4.42.x. May confirm or refute the watchdog hypothesis cheaply.
4. **NOT viable: rebuilding inside Nickel.** Cascading taxonomy and custom welcome screen with reader name are both impossible in Nickel; Plato is structurally required for Sangala's UX. (Research summary in this session 2026-05-25.)
5. **Do NOT do the v2.24 rewrite or any other "rebuild on an old base"** — every old base also fails.
6. **The v2.49 production batch (~30 devices) is at risk.** Field deployments may start factory-resetting once users cumulatively power-cycle enough. If field reports come in, the answer is the architectural fix, not an incremental patch.

### Conversation-level conventions (carry-forward)

- The user has explicitly pushed back this session on:
  - Assuming user approval to multi-part proposals when only one part was confirmed
  - Asking questions the assistant could answer from git/records itself
  - Tone that reads as commanding ("Stop" etc.)
- Behaviors to maintain: state findings directly, ask one focused question at a time, verify against git rather than asking the user to recall.
...
- **Factory reset on power-cycle (TOP PRIORITY, UNRESOLVED).** Reproduces on the ~2nd power cycle. Confirmed across hardware: original test device N365594030627 AND a new never-previously-reset Clara BW both reset. Confirmed across versions: v2.24 manual drag-drop through v2.53 CLI, including v2.47 (the previously claimed stable baseline). Five hypotheses tested and disproven (GUI installer, boot-grace skip, post-v2.24 changes, USB cable, device-specific hardware/eMMC). The bug is in v2.47 itself or its install process or its interaction with Kobo firmware 4.42.23291. **Do not tag any version as stable.** Next step is genuine root-cause investigation — possibly comparing what changed between earlier "actually stable" eras (v2.30, v2.32) and v2.47 in `plato-autostart.sh`, KoboRoot.tgz, or the install package layout. External resources (MobileRead forums, Kobo firmware behavior) may help.
- **Fresh install hang**: fixed in v2.32. Validated on factory-reset Clara BW (2026-05-06). v2.44+'s no-grace-on-subsequent-boots optimization preserves the factory-reset path unchanged. (Note: "v2.32 stable" claim now suspect per Lesson #33 — was never multi-power-cycle-tested.)
...
32. **Kobo Clara BW devices factory-reset after a few power cycles — bug is structural to our Plato + plato-autostart.sh + minimal KoboRoot.tgz architecture, not regression-introduced.** Symptom: device factory-resets on the 2nd–4th power cycle, enters recovery mode, lands at Kobo's OOBE wizard. **Hypotheses tested and disproven** as of 2026-05-25: (a) ~~GUI installer is the cause~~ (CLI also resets); (b) ~~Anything we added since v2.24 is the cause~~ (v2.24 manual drag-drop also fails); (c) ~~Flaky USB cable causes the reset~~ (Lesson #30 — cable fixes install-time frozen dots only); (d) ~~Test-device hardware fault or eMMC wear~~ (multiple fresh devices reset); (e) ~~Post-v2.47 UI changes~~ (v2.47 fails without them); (f) ~~WiFi tamper-detect~~ (never toggled); (g) ~~Host-side install logic~~ (manual drag-drop also fails); (h) ~~Anything between v2.32 and v2.47~~ (v2.32 ALSO factory-resets on the 4th power cycle on a fresh device — see Lesson #35 update). **The bug has been latent in the project since at least v2.32**, never caught because no original "stable" verification did multi-power-cycle testing in close succession (Lesson #33). v2.32 was the earliest version we ever called stable, and it also fails. Most likely root cause: Kobo's bootloader has a boot-attempt-counter mechanism that resets on a "healthy" Nickel session; every boot under our architecture kills Nickel via `plato-autostart.sh` + `plato.sh` before it can write whatever marker Kobo wants, eventually tripping recovery. Cannot confirm without device-side logs (which are wiped on reset). Architectural fix candidates: (i) reintroduce KFMon-based launching (upstream Plato community standard — doesn't kill Nickel, adds an icon that launches Plato as a separate process; requires removing `plato-autostart.sh` to avoid v2.28's race condition per Lesson #13); (ii) rebuild Sangala UX inside Nickel instead of replacing it. Do not tag any new version as stable until a device survives 10+ power cycles.
33. **Don't declare a version "stable" from a successful install alone — power-cycle 10+ times in close succession.** v2.49's production batch was marked stable based on ~30 devices "installing cleanly." None were tested under repeated power-cycling. v2.49 is now confirmed (2026-05-23) to factory-reset on ~3rd power cycle on the test device, and the production-batch devices are presumed at-risk. Same for v2.48, v2.32, v2.30 — all marked stable based on install-and-boot-once tests. **v2.32 was tested under the proper protocol on 2026-05-25 and factory-reset on the 4th power cycle** (open a book, check a definition, power off, 20–30s wait, repeat). Every "stability" claim in CLAUDE-STATE prior to 2026-05-25 should be considered wrong. The minimum bar for any future stability claim is: install on a fresh device → 10+ power cycles with normal usage in between → no reset. Until something passes that, treat everything as broken.
34. **Pull a device-state snapshot before declaring "device is fine."** The snippet under "Pull a snapshot of the device's state to your host" in the 2026-05-23 session captures `info.log`, `dictionary.log`, `Settings.toml`, the contents of `.kobo/` (minus screensaver), and SHA256 hashes of key files (`plato` binary, `plato.sh`, `Settings.toml`, `Kobo eReader.conf`, `version`). Run it after each boot to see what's drifting between boots — Kobo writes to `.kobo/version` on each boot, Plato writes to `Settings.toml`, and Nickel can touch files during its brief pre-plato-autostart window. Worth committing to the repo as `tools/snapshot-device.ps1` in a future session.
35. **Multi-power-cycle test reveals the bug has been latent the entire project — v2.32 also factory-resets.** (2026-05-25) A cable swap resolved install-time frozen-dots (Lesson #30). Then on fresh, never-reset Clara BW devices: (a) v2.47 CLI install reset on cycle 2; (b) v2.47 manual install reset on cycle 4; (c) v2.32 manual install with normal usage (open book, check definition, power off, 20–30s wait, repeat) reset on cycle 4. WiFi never toggled in any run. v2.32 was the earliest version we ever called stable; it also fails. This means: (1) every install path produces the reset (CLI/GUI/manual); (2) every version we've tagged "stable" was incompletely validated; (3) the bug is structural — in the kill-Nickel-on-every-boot architecture, not in any specific code change. The most likely root cause is a Kobo bootloader watchdog that counts boot-attempts and trips recovery when too many "unhealthy" boots accumulate without a clean Nickel session writing a "this boot succeeded" marker. Per Lesson #36, Nickel cannot replicate Sangala's UX (hierarchical taxonomy and welcome-screen-with-name are impossible there), so the fix has to be in how we launch Plato — most likely restoring KFMon-based launching that doesn't kill Nickel (Lesson #13). Never assume a "stable" version is actually stable without 10+ close-succession power cycles; always verify version contents from `git show` rather than CLAUDE-STATE prose.

36. **Nickel cannot replicate Sangala's UX — Plato is structurally required.** (2026-05-25 research) Per-requirement verdict on stock Nickel + NickelMenu + kobopatch combinations: (1) cascading hierarchical taxonomy — NOT POSSIBLE, Kobo collections are flat tags with no nesting (MobileRead "Nested Collections?" threads); (2) welcome screen with reader's name — NOT POSSIBLE, home screen is hard-coded, only screensaver is replaceable; (3) restricted menu (remove Sync/WiFi/Store/etc.) — kobopatch firmware binary patches only, per-firmware, fragile, Clara BW FW 4.39+ coverage is limited; (4) Wiktionary dictionary — dictutil/Penelope can produce a Kobo dict file, but conversion is desktop-only, no on-device StarDict conversion; (5) Libertinus Serif font — native, custom fonts in `.kobo/fonts/` work, but pinch-to-zoom step size is Kobo-controlled (integer points, no 0.5pt steps); (6) no clock/date/Share/Sleep prompts — kobopatch only; (7) dot-prefixed folders hidden from scanner — native via `[FeatureSettings] ExcludeSyncFolders=...` regex in `.kobo/Kobo/Kobo eReader.conf` (already set in Sangala's config). Requirements 1 and 2 alone make Nickel non-viable. Do not propose "rebuild Sangala UX inside Nickel" as a path forward — Plato (or KOReader) is required. The factory-reset fix has to be in how we launch Plato, not in whether we use it.
