...
**v2.53-sangala** (next ship; pre-release). Single critical change on top of v2.52:

4. **Revert `2566fc7` ("Skip the 5s post-DB grace on subsequent boots").** Prime suspect for the factory-reset bug. v2.32's behavior restored: every boot waits up to 60s for `KoboReader.sqlite`, then a 5s grace, then kills Nickel. v2.32-v2.43 had this behavior and the user remembers no factory resets in that era. v2.44 changed to "kill Nickel immediately on subsequent boots" — flagged "**Untested on device**" in CLAUDE-STATE at the time. Both v2.49 GUI and CLI installs are now confirmed to factory-reset on power-cycle (user report 2026-05-23); the only on-device script change between v2.32 (no resets) and v2.49 (resets) is `2566fc7`. **Trade-off accepted:** Nickel is briefly visible (~5s) on every boot in exchange for no factory resets. Bricked devices >> seeing Nickel for a few seconds. **This is a hypothesis, not a proven fix.** Device test must include multiple power cycles AND a WiFi-on cycle before declaring it fixed.

**v2.51-sangala** (pre-release). Three changes on top of v2.50:

1. **Drop the GUI `.exe` from `install-sangala.zip`.** Releases ship the CLI `.ps1` only, like v2.48 did. The `gui-exe` job is renamed `gui-exe-parse-check` and no longer uploads to the release — it just verifies the GUI source still PS2EXE-compiles, so we catch regressions if we ever resume shipping it. Motivation: production devices installed via the GUI `.exe` have intermittently performed real factory resets (recovery → OOBE wizard). **NOTE (2026-05-23):** later evidence shows CLI installs of v2.49 also factory-reset, so the GUI is not the culprit. See Lesson #32 update. The CLI-only zip change still stands (no reason to ship a broken GUI), but it's no longer believed to be the fix for resets.
...
3. **12-hour clock with AM/PM, simplified clock-menu.** Top-bar clock now shows `3:04 PM` rather than `15:04` (default `time_format` changed to `"%-I:%M %p"` in `crates/core/src/settings/mod.rs` and overridden in `sangala/Settings.toml`). Clock dropdown menu reworked to three submenus that all fit on screen without `More`-overflow: `AM / PM` (2 entries), `Set Hour` (12 entries, 1–12), `Set Minute` (6 buckets `00-09 / 10-19 / .. / 50-59`, each opening a 10-entry leaf submenu — 3 taps to pick any minute, all leaf lists short enough to fit). Removed the date-format message header from the menu — date is not displayed or settable. New `SetTimeAmPm(bool)` event variant; `SetTimeHour` handler now maps a 1..=12 value to the 24h hour preserving the current AM/PM. This fixes user-reported problems with the previous menu: 24h-only display, no AM/PM toggle, minute granularity was step-5 only, and the appearance that the hour list was "not sequential" (caused by the 24-entry hour list overflowing into nested `More` submenus during navigation — gone now because every submenu fits).
...
32. **Production Kobo Clara BW devices have been performing real factory resets (recovery mode → OOBE wizard, user partition wiped) after install.** Often immediately on next power-cycle; sometimes after days of normal use; sometimes triggered by a WiFi connection attempt. **First-pass hypothesis: GUI installer is the cause.** Diff'd CLI vs GUI installers and found two GUI-specific deltas (background runspace for copy; 500ms reconnect polling vs CLI's 3s). v2.51 shipped the conservative response: drop the GUI `.exe` from `install-sangala.zip` entirely. **Hypothesis disproven 2026-05-23:** user reported a v2.49 CLI install also factory-reset on power-cycle. Both installer types are affected → the cause is on-device, not in the host-side installer. **Current working hypothesis: commit `2566fc7` (v2.44) which skipped the 5-second post-DB grace on subsequent boots is the cause.** That commit was the only on-device script change between v2.32 (user remembers no resets in that era) and v2.49 (resets confirmed). Behind the change: v2.32 always waited for `KoboReader.sqlite` to exist then slept 5s before killing Nickel. v2.44 changed subsequent boots to kill Nickel ~1-3s after `pidof nickel` succeeds — Nickel may not have completed its boot-time integrity checks in that window, leaving the system in a state that the Kobo bootloader eventually escalates to a recovery/factory-reset. v2.53 reverts `2566fc7`. **Device test must include multiple power cycles and a WiFi-on cycle** — symptom is sometimes only visible after the Nth boot. Cannot get device-side logs (factory reset wipes the user partition), so confirmation has to be empirical: ship v2.53 to a small set of devices and observe whether resets stop. The v2.51 GUI-drop and WiFi-menu-removal changes stand on their own merits (no reason to ship a suspect installer or a menu path that may correlate with the reset trigger), but they're no longer believed to be the fix.
