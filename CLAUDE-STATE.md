...
- **Home landing page**: Implemented in `Shelf::update`'s empty-state branch (no overlay). When the active library has no visible books, Shelf pushes an `Image` view (top 2/3 of its rect, scaled to fit via the document framework) and a centered `Label` ("Welcome, {welcome-name}!") in the bottom 1/3 instead of the white filler. Gated on both `home-image` and `welcome-name` being set in `[home]`; falls back to white filler if either is missing or the image fails to load. The dead `HomeImage` overlay view and its `remove_home_image` plumbing were deleted. Side effect to be aware of: any time Shelf is rebuilt with zero metadata (e.g., a filter that returns no results in a populated library), the welcome screen will render — for Sangala this only matters for the empty-by-design Menu library.
