...
**USB cable swap.** Per Lesson #30, this exact symptom (`updating` → frozen dots) was historically a flaky USB cable on the v2.49 production batch; cable swap resolved every recurrence. The same cable has been used for every install in this session across multiple devices. Intermittent cable behavior (Device 1 succeeds, Device 2 freezes with same cable) is consistent with the v2.49 production-batch experience. **This should be the first thing tried in the next session.**
...
1. **Flaky USB cable / hardware-level transfer corruption** (Lesson #30 pattern). Highest plausibility; cheapest to test. Not yet tested with a swap.
2. **Different USB port on the host** — also untested.
3. **Test device's eMMC flash degrading** — many factory resets and installs over the session history; write cycle wear is possible though probably not at this scale.
4. **Test device hardware fault** specific to this unit — needs a second Clara BW to differentiate.
5. **Something we haven't thought of.** External resources (Kobo firmware documentation, MobileRead forums, etc.) might shed light.
...
1. **Try a different USB cable on a fresh factory-reset device** with the existing v2.52 release (or whatever's marked Latest). This is the only untested hypothesis that matches all the symptom patterns. ~5 minutes to test.
...
- **Factory reset on power-cycle (TOP PRIORITY, UNRESOLVED).** Test device factory-resets on ~3rd power cycle regardless of installed version (v2.24 manual drag-drop through v2.53 CLI). Three hypotheses tested and disproven this session (see Lesson #32). **Untested:** USB cable swap. This is the next thing to try before any code changes.
...
32. **Kobo Clara BW devices factory-reset on boot after installing this build; root cause not yet found.** Symptom: device crashes during boot (often the ~3rd power cycle), enters recovery mode, factory-resets to Kobo's OOBE wizard. Sometimes installs land cleanly then fail on later boots; sometimes the first post-install boot itself fails with frozen non-animated loading dots after the "updating" screen. **Three explicit hypotheses have been tested and disproven on the test device** (Clara BW, serial `N365594030627`, firmware `4.42.23291` — unchanged across all tests): (a) ~~GUI installer is the cause~~ (v2.49 CLI install also resets); (b) ~~Boot-grace skip in v2.44's commit `2566fc7` is the cause~~ (reverted in commit `ebcf55c` on branch; device still resets); (c) ~~Anything we added since v2.24 is the cause~~ (v2.24 itself installed via plain manual drag-drop and Windows' Safely Remove Hardware produces the same frozen-dots symptom on this device). **Highest remaining hypothesis (untested in this investigation): flaky USB cable** — Lesson #30's pattern (`updating screen → frozen dots`) was previously diagnosed as a cable in the v2.49 production batch and the cable swap resolved every recurrence; in this session the same cable was used for every install across multiple devices, and we never tested with a different cable. **Other untested candidates:** different USB port on host, eMMC wear from many install/reset cycles on the same test device, hardware fault specific to this unit. Cannot get device-side logs after a reset (user partition is wiped); the host-side `install-sangala.log` consistently looks clean even on devices that subsequently reset. **For the next session: try a different USB cable first, before any code changes.** Do not tag any v2.5x with a "fix" for this until at least one device survives 10+ power cycles plus a sleep/wake cycle without resetting.
