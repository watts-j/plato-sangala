...
**IMPORTANT**: All development happens on branch `claude/customize-plato-ui-1Edbm`. Run `git checkout claude/customize-plato-ui-1Edbm` before making any changes. The version numbers (v2.20, v2.27, etc.) are git tags on that branch, not Plato's upstream version.
